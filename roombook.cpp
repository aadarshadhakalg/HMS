...
void MainWindow::packageprice_adder()
{
    ...
    QSqlQuery pkg_query(db);
    bool contains = false;
    if(roomBtn_select == 1){
    pkg_query.prepare("SELECT * FROM packages");
    pkg_query.exec();
    while(pkg_query.next()){
        if(package_id->text().toInt() == pkg_query.value(0).toInt()){
            contains = true;
        }
    }

    if(contains == false){
        QString msg = "Entered Package Id not found on available Package List";
        throw msg;
    }
...
void MainWindow::bookButton_clicked(){
    ...
    if(!emailrx.match(email).hasMatch()){
        QString msg = "OOPS! Email address is not valid";
        throw msg;
